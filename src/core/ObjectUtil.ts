 ...
 * - Null-case Handling
 */
function safeAssign<T extends object | undefined | null>(
    object: T,
    updatedFields: Partial<NonNullable<T>> | undefined | null,
): T {
    if (object) {
 ...
 */
function toObject<T extends object, V>(
    object: T,
 ...
 * E.g: {a: 10, b: undefined} is treated as {a: 10}.
 */
function toArray<T extends object, V>(
    object: T,
    mapperCallback: (key: keyof T & string, value: WithoutUndefined<T[keyof T]>, index: number) => WithoutUndefined<V>,
): WithoutUndefined<V>[] {
    const result: WithoutUndefined<V>[] = [];
    Object.keys(object).forEach(
        (key, index) =>
            object[key] !== undefined && result.push(mapperCallback(key as keyof T & string, object[key], index)),
    );
    return result;
}
...
function forEach<T extends object>(
    object: T,
    forEachCallback: (key: keyof T & string, value: WithoutUndefined<T[keyof T]>, index: number) => any,
): void {
...
function deepClone<T extends object>(object: T): T {
    const innerDeepClone = (innerObject: any, isInnerLevel: boolean = true): any => {
        ...
        if (innerObject instanceof Object) {
            const newObject: any = {};
            // eslint-disable-next-line no-restricted-syntax -- using for-in loop requires checking object.hasOwnProperty to exclude properties up from the prototype chain
...
function sortByKeys<T extends object>(object: T, priorityList: readonly (keyof T)[]): T {
    const objectKeys = Object.keys(object) as (keyof T)[];
    objectKeys.sort((a, b) => {
