...
function sumByKey<T>(array: readonly T[], key: keyof T): number {
    let sum = 0;
 ...
 */
function toggleElement<T>(array: readonly T[], element: T): T[] {
...
function areSame<T>(a: readonly T[], b: readonly T[], compareOrdering: boolean): boolean {
 ...
 */
function sortBy<T>(array: readonly T[], ...priorityLists: (readonly T[])[]): T[] {
 ...
 * If more than 1 `priorityLists` are provided, the latter takes precedence over the previous.
 */
function sortByKey<T extends object, K extends keyof T>(
    array: readonly T[],
    key: K,
    ...priorityLists: (readonly T[K][])[]
): T[] {
    const reversedPrioritySet = priorityLists.reduce((prev, curr) => [...curr, ...prev], []);
    const mergedPriorityList = [...new Set(reversedPrioritySet)];
...
function chunk<T>(array: readonly T[], sizeOrShape: number | readonly number[]): T[][] {
    ...
    if (typeof sizeOrShape === "number") {
        ...
        if (shapeTotal !== array.length) {
            throw new Error(
                `[util] ArrayUtil.chunk: number group shape total ${shapeTotal} does not equal to array length ${array.length}`,
            );
        }
...
function hasIntersection<T>(a: readonly T[], b: readonly T[]): boolean {
 ...
 */
function compactMap<T, V>(array: readonly T[], callback: (item: T, index: number) => V): NonNullable<V>[] {
    return array
        .map(callback)
        .filter(
